...
class TenantService {
  ...
  async getAllTenants() {
    const tenants = await prisma.tenants.findMany({
      ...
      include: {
        users: {
          // Include the user data (name, phone, email, etc.)
          select: {
            user_id: true,
            full_name: true,
            phone: true,
        ...
        rooms: {
          // Include the room they are linked to
          select: {
            room_id: true,
            room_number: true,
            floor: true,
  ...
  async getTenantChatbotContext(tenantUserId) {
    ...
    const activeContracts = await prisma.contracts.findMany({
      ...
      include: {
        room_history: {
          // The room associated with contract
          include: {
            building: {
              include: {
                building_managers: {
    ...
    return {
      ...
      finance: {
        unpaid_bills_count: recentBills.filter(
      ...
      vehicles: {
        ...
        active_vehicles: activeVehicles.map((v) => ({
          ...
          parking_slot: v.slot
            ? `${v.slot.slot_number} (${v.slot.building?.name})`
            : "No Slot Assigned",
        })),
      },
  ...
  async lookupTenantByExactInfo(identifier) {
          ...
          include: {
            room: {
              select: {
                ...
                building_id: true,
                building: {
                  // relation trong model rooms là 'building'
                  select: { name: true },
                },
              },
            },
    ...
    // Nếu tenant đang là secondary (is_current=true) ở bất kỳ phòng nào => không cho lookup thấy nữa
    ...
    if (existingSecondary) {
      // Trả null để controller trả 404 => FE coi như không tìm thấy
      return null;
    }
  ...
  async findBestMatchTenant(searchData) {
    const { tenant_name, tenant_phone, tenant_id_number, room_number } =
      searchData;
    ...
    const scoredCandidates = candidates.map((candidate) => {
      ...
      if (tenant_phone && candidate.users.phone) {
        ...
        if (
          inputPhone &&
          dbPhone &&
          (dbPhone.includes(inputPhone) || inputPhone.includes(dbPhone))
        ) {
          score += 30;
