...
async function main() {
  ...
  let tenantAlice = await prisma.tenants.findFirst({
    where: { id_number: "TNT-0001" },
  });
  if (!tenantAlice) {
  ...
  const existingOwner = await prisma.building_owner.findUnique({
  ...
  // --- BILL (issued) ---
  let bill = await prisma.bills.findUnique({
    where: { bill_number: "BILL-2025-0001" },
  });
  if (!bill) {
  ...
  let payment = await prisma.bill_payments.findFirst({
    where: { reference: "TXN-20251013-0001" },
  });
  if (!payment) {
  ...
  const grExists = await prisma.guest_registrations.findFirst({
    where: { host_user_id: alice.user_id },
  });
  if (!grExists) {
